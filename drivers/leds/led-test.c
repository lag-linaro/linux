...
static void led_test_class_register(struct kunit *test)
{
	...
	KUNIT_ASSERT_EQ(test, cdev->max_brightness, LED_FULL);
	KUNIT_ASSERT_EQ(test, cdev->brightness, LED_TEST_POST_REG_BRIGHTNESS);
	KUNIT_ASSERT_STREQ(test, cdev->dev->kobj.name, "led-test");
	...
	KUNIT_ASSERT_STREQ(test, cdev_clash->dev->kobj.name, "led-test_1");
	KUNIT_ASSERT_STREQ(test, cdev_clash->name, "led-test");
...
static void led_test_class_add_lookup_and_get(struct kunit *test)
{
	...
	KUNIT_ASSERT_STREQ(test, cdev_get->name, cdev->name);
